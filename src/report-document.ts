...
export async function buildReportDocumentHtml(reportId: string) {
  const [{ data: report, error: reportError }, { data: answers, error: answersError }] = await Promise.all([
    ...
    supabase.from('audit_answers_final').select('question_id, value, observation, evidence_url, evidence_urls, numeric_value_theoretical, numeric_value_physical, numeric_value_current, numeric_value_previous, numeric_items, checklist_questions(question_text, sort_order, question_type)').eq('report_id', reportId),
  ]);
  ...
  const rows = signedAnswers.map((answer, index) => {
    ...
    return `<section><h3>${index + 1}. ${escapeHtml(question)}</h3><p><strong>Respuesta:</strong> ${escapeHtml(answer.value === 'cumple' ? 'Cumple' : answer.value === 'no_cumple' ? 'No cumple' : answer.value)}</p>${answer.observation ? `<p><strong>Observación:</strong> ${escapeHtml(answer.observation)}</p>` : ''}${evidenceHtml}</section>`;
  }).join('');
