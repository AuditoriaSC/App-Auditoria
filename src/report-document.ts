...
export async function buildReportDocumentHtml(reportId: string) {
  const [{ data: report, error: reportError }, { data: answers, error: answersError }] = await Promise.all([
    supabase.from('audit_reports').select('id, region, visit_type_id, local_name_snapshot, local_code_snapshot, auditor_name_snapshot, responsible_name_snapshot, start_date, start_time, end_time, final_grade, final_percentage, signature_auditor_url, signature_responsible_url, auditor_signature_url, responsible_signature_url, edited_after_send, last_edit_reason, resent_count, updated_at').eq('id', reportId).single(),
    supabase.from('audit_answers_final').select('question_id, value, observation, evidence_url, numeric_value_theoretical, numeric_value_physical, numeric_value_current, numeric_value_previous, numeric_items, checklist_questions(question_text, sort_order, question_type)').eq('report_id', reportId),
  ]);
  ...
  const signedAnswers = await Promise.all((answers || []).map(async (answer) => ({ ...answer, evidence: answer.evidence_url ? await signedEvidenceUrl(answer.evidence_url, SIX_MONTHS_SECONDS) : null })));
  ...
  const rows = signedAnswers.map((answer, index) => {
    ...
    return `<section><h3>${index + 1}. ${escapeHtml(question)}</h3><p><strong>Respuesta:</strong> ${escapeHtml(answer.value === 'cumple' ? 'Cumple' : answer.value === 'no_cumple' ? 'No cumple' : answer.value)}</p>${answer.observation ? `<p><strong>Observación:</strong> ${escapeHtml(answer.observation)}</p>` : ''}${answer.evidence ? `<img src="${escapeHtml(answer.evidence)}" alt="Evidencia" />` : ''}</section>`;
  }).join('');

  return `<!doctype html><html><head><meta charset="utf-8"><title>Informe ${escapeHtml(report.local_name_snapshot)}</title><style>@page{size:A4;margin:16mm}body{font-family:Arial,sans-serif;color:#2B2118;line-height:1.45}header{background:#165034;color:white;padding:18px;border-radius:10px}h1{margin:0;font-size:22px}.meta{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin:18px 0;padding:12px;background:#FBF8F1}section{break-inside:avoid;border:1px solid #DED2C2;border-radius:8px;padding:12px;margin:10px 0}section h3{color:#165034;font-size:15px;margin:0 0 8px}img{display:block;max-width:520px;max-height:360px;margin:10px auto;border-radius:6px}.signatures{display:flex;gap:20px;margin-top:18px}.signature{width:50%;text-align:center}.signature img{max-height:120px}</style></head><body><header><h1>Informe de visita ${escapeHtml(report.visit_type_id)}</h1><div>${escapeHtml(report.local_name_snapshot)} · ${escapeHtml(report.local_code_snapshot)}</div></header><div class="meta"><div><strong>Auditor:</strong> ${escapeHtml(report.auditor_name_snapshot)}</div><div><strong>Responsable:</strong> ${escapeHtml(report.responsible_name_snapshot)}</div><div><strong>Fecha:</strong> ${escapeHtml(report.start_date)}</div><div><strong>Horario:</strong> ${escapeHtml(report.start_time)} - ${escapeHtml(report.end_time)}</div><div><strong>Calificación:</strong> ${Number(report.final_grade || 0).toFixed(2)} / 10</div><div><strong>Cumplimiento:</strong> ${Number(report.final_percentage || 0).toFixed(2)}%</div></div>${rows}<div class="signatures"><div class="signature"><strong>Firma auditor</strong>${auditorSignatureUrl ? `<img src="${escapeHtml(auditorSignatureUrl)}" />` : '<p>Sin firma</p>'}</div><div class="signature"><strong>Firma responsable</strong>${responsibleSignatureUrl ? `<img src="${escapeHtml(responsibleSignatureUrl)}" />` : '<p>Sin firma</p>'}</div></div>${report.edited_after_send ? `<p><strong>Editado:</strong> ${escapeHtml(report.last_edit_reason)} · Reenvíos: ${Number(report.resent_count || 0)}</p>` : ''}</body></html>`;
}
